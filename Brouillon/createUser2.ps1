class ADadmin {
    ...
    [void] creationGroup([string]$service) {
        ...
        catch { # Si la recherche genere une erreur (echoue) on la capture et on crée le group
...
} while ($srvADDS -eq $null) #La boucle se reexcute jusqu'a ce que $srvADDS ai une valeur differente de $null (aucune valeur)
Start-Sleep -Seconds 1
Clear-Host
$srvADDS.choixAction() 